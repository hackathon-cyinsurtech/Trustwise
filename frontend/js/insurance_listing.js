function getDuration (seconds) {
    ...
    if (seconds<60*60*60) return Math.floor(seconds/60/60).toString() + " hour(s)"
    if (seconds<60*60*60*24) return Math.floor(seconds/60/60/24).toString() + " days(s)"
    if (seconds<60*60*60*24*30) return Math.floor(seconds/60/60/24/30).toString() + " month(s)"
    return Math.floor(seconds/60/60/60/24/365).toString() + " year(s)"
}

var statusEnum = {
    1: {class: "list-group-item-primary", name: "bidding"},
    2: {class: "list-group-item-success", name: "active"},
    3: {class: "list-group-item-warning", name: "expired"},
    4: {class: "list-group-item-danger", name: "triggered"},
    5: {class: "list-group-item-dark", name: "cancelled"}
};

function createInsuranceListItem(hash, status, premium, payout, start, end, description, isBelow, temp) {
    getContractState(hash).then(status => {
    var newListItem = '';
...
$(document).ready(function() {
  getAllInsurances().then(vv => {
    console.log(vv);
    vv.forEach(v => {
      createInsuranceListItem(v.instantiation, 1, web3.fromWei(v.lowestPremium), web3.fromWei(v.payout), v.startTime, v.endTime, v.description, v.isTempBelow, v.temperature)
    })
  })
});
